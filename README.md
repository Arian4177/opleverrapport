[opleverrapport-badkamer.html](https://github.com/user-attachments/files/29340225/opleverrapport-badkamer.html)
<!DOCTYPE html>
<html lang="nl">
<head>
<meta charset="UTF-8">
...
const CLOUDINARY_CLOUD = 'dyyhyficv';
const CLOUDINARY_PRESET = 'wbp_oplever';
...
function handleApPhoto(key, input) {
  ...
  reader.onload = e => { photoData[key] = e.target.result; buildApPhotos(); };
...
async function submitForm() {
  ...
  // 1. Genereer PDF en upload naar Filestack (geen AI meer nodig)
  ...
  try {
    ...
    zipForm.append('file', zipBlob, `Fotos_${klant}_${new Date().toISOString().slice(0,10)}.zip`);
    zipForm.append('upload_preset', CLOUDINARY_PRESET);
    zipForm.append('resource_type', 'raw');
    const zipRes = await fetch(`https://api.cloudinary.com/v1_1/${CLOUDINARY_CLOUD}/raw/upload`, {
      method: 'POST', body: zipForm
    });
    const zipData = await zipRes.json();
    zipLink = zipData.secure_url || '';
  ...
  try {
    ...
    const emailParams = {
      ...
      pdf_link:     pdfLink ? `Download PDF: ${pdfLink}` : 'PDF niet beschikbaar',
      zip_link:     zipLink ? `Download alle foto's (ZIP): ${zipLink}` : 'Geen foto\'s beschikbaar',
    };
    await emailjs.send(EMAILJS_SERVICE_ID, EMAILJS_TEMPLATE_ID, emailParams);
    await emailjs.send(EMAILJS_SERVICE_ID, EMAILJS_TEMPLATE_ID, { ...emailParams, to_email: 'badkamertechniek@visserpoortvliet.nl' });
  } catch(e) {
...
function downloadPDF() {
  ...
  // ── AI SAMENVATTING ──
  const samenvattingTxt = document.getElementById('ai-summary').textContent;
