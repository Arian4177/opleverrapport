...
<title>Opleverrapport Badkamer — WBP Poortvliet</title>
...
<div id="setup-screen" class="setup-screen">
  <div class="setup-logo">WBP Poortvliet</div>
...
<div id="main-app" style="display:none">
  ...
  <div class="header">
    <div class="header-top">
      <div class="header-logo">WBP Poortvliet</div>
  ...
  <div class="step" id="step-3">
    ...
    <div class="nav-bar">
      ...
      <button class="btn-primary" onclick="naarStap3b()">Volgende →</button>
    </div>
  </div>

  <!-- Step 3b: Aandachtspunt photos -->
  <div class="step" id="step-3b">
    <div style="padding:16px 20px 8px">
      <div style="font-size:16px;font-weight:600;color:var(--gray-900);margin-bottom:4px">📸 Foto's verplicht</div>
      <div style="font-size:13px;color:var(--gray-400)">Maak van elk aandachtspunt en niet OK onderdeel een foto. Dit is verplicht.</div>
    </div>
    <div id="aandachtspunt-waarschuwing" style="display:none;margin:0 20px 8px;padding:12px 14px;background:#FAEEDA;border-radius:10px;font-size:13px;color:#854F0B;line-height:1.5"></div>
    <div class="photo-grid" id="ap-photo-grid"></div>
    <div class="nav-bar">
      <button class="btn-secondary" onclick="goStep(3)">←</button>
      <button class="btn-primary" onclick="checkAandachtspuntFotos()">Volgende →</button>
    </div>
  </div>
  ...
  <div class="step" id="step-4">
    ...
    <div class="nav-bar">
      <button class="btn-secondary" onclick="goStap4Terug()">←</button>
  ...
  <div class="step" id="step-6">
    <div id="sending-view" class="center-screen">
      ...
      <div style="font-size:14px;color:var(--gray-400);margin-top:6px">Even geduld...</div>
    </div>
    <div id="done-view" class="center-screen" style="display:none">
      ...
      <button class="btn-primary" style="margin-top:1.5rem;width:100%" onclick="downloadPDF()">📄 Download PDF rapport</button>
      <button class="btn-primary" style="margin-top:10px;width:100%;background:#2F5496" onclick="downloadFotos()">📥 Download alle foto's</button>
      <button class="btn-secondary" style="margin-top:10px;width:100%;padding:15px;border-radius:10px;font-size:16px;cursor:pointer;font-family:'DM Sans',sans-serif" onclick="nieuwRapport()">Nieuw rapport starten</button>
    </div>
  </div>
...
<script>
// ============================================================
// GOOGLE APPS SCRIPT CONFIGURATIE
// De PDF en foto's worden als bijlage gemaild via een gratis
// Google Apps Script. Plak hieronder de webapp-URL die je krijgt
// na het publiceren van het script (eindigt op /exec).
// ============================================================
const SCRIPT_URL = 'https://script.google.com/macros/s/AKfycby4JH_VmmMTwXU5t8UV9_J-m1oRy74AAwaSx1yEKt3p3Z2HowtBkYiDkT_M9_8u3s9e/exec';

// Wie ontvangt het rapport (met PDF + foto's als bijlage):
const ONTVANGERS = [
  'baderie@wbpoortvliet.nl'
  // 'badkamertechniek@visserpoortvliet.nl'  // <- testfase: tijdelijk uit. Verwijder de // voor de // om Visser weer aan te zetten
];
...
function buildPhotos() {
  ...
  photoNames.forEach(name => {
    if (!photoData[name]) photoData[name] = null;
...
function buildApPhotos() {
  const g = document.getElementById('ap-photo-grid');
  g.innerHTML = '';
  const verplicht = checkItems.filter(i => checkData[i] && (checkData[i].status === 'pm' || checkData[i].status === 'nok'));
  verplicht.forEach(item => {
    const key = 'ap-' + item;
    if (photoData[key] === undefined) photoData[key] = null;
    const isPm = checkData[item].status === 'pm';
    const borderColor = isPm ? '#BA7517' : '#E24B4A';
    const bgColor = isPm ? '#FAEEDA' : '#FCEBEB';
    const icon = isPm ? '⚠️' : '✗';
    const card = document.createElement('div');
    card.className = 'photo-card' + (photoData[key] ? ' taken' : '');
    card.id = 'pc-' + key;
    if (!photoData[key]) { card.style.borderColor = borderColor; card.style.background = bgColor; }
    if (photoData[key]) {
      card.innerHTML = `<img src="${photoData[key]}" alt="${item}"><div class="taken-badge">✓</div><div style="position:absolute;bottom:0;left:0;right:0;text-align:center;font-size:10px;color:#fff;background:rgba(0,0,0,0.45);padding:3px 4px">${item}</div><input type="file" accept="image/*" capture="environment" onchange="handleApPhoto('${key}',this)">`;
    } else {
      card.innerHTML = `<div style="font-size:22px">${icon}</div><div style="font-size:11px;color:${isPm?'#854F0B':'#A32D2D'};text-align:center;padding:0 6px;font-weight:500;line-height:1.3">${item}</div><input type="file" accept="image/*" capture="environment" onchange="handleApPhoto('${key}',this)">`;
    }
    g.appendChild(card);
  });
  updateAandachtspuntWaarschuwing();
}

function handleApPhoto(key, input) {
  if (!input.files[0]) return;
  const reader = new FileReader();
  reader.onload = e => { resizeImage(e.target.result, 1600, 0.85, r => { photoData[key] = r; buildApPhotos(); }); };
  reader.readAsDataURL(input.files[0]);
}

function updateAandachtspuntWaarschuwing() {
  const ontbrekend = checkItems.filter(i => checkData[i] && (checkData[i].status === 'pm' || checkData[i].status === 'nok') && !photoData['ap-' + i]);
  const el = document.getElementById('aandachtspunt-waarschuwing');
  if (!el) return;
  if (ontbrekend.length > 0) {
    el.style.display = 'block';
    el.innerHTML = `<strong>📸 Foto nog vereist voor:</strong><br>${ontbrekend.map(i => '• ' + i).join('<br>')}`;
  } else {
...
function naarStap3b() {
  const verplicht = checkItems.filter(i => checkData[i] && (checkData[i].status === 'pm' || checkData[i].status === 'nok'));
  if (verplicht.length === 0) { goStep(4); return; }
  document.querySelectorAll('.step').forEach(s => s.classList.remove('active'));
  document.getElementById('step-3b').classList.add('active');
  document.getElementById('header-title').textContent = "Foto's verplicht";
  document.getElementById('step-name').textContent = 'Stap 3b van 6';
  document.getElementById('progress-fill').style.width = '42%';
  buildApPhotos();
  window.scrollTo(0, 0);
}

function checkAandachtspuntFotos() {
  const ontbrekend = checkItems.filter(i => checkData[i] && (checkData[i].status === 'pm' || checkData[i].status === 'nok') && !photoData['ap-' + i]);
  if (ontbrekend.length > 0) {
    updateAandachtspuntWaarschuwing();
    document.getElementById('aandachtspunt-waarschuwing').scrollIntoView({ behavior: 'smooth' });
    return;
  }
  goStep(4);
}

function goStap4Terug() {
  const verplicht = checkItems.filter(i => checkData[i] && (checkData[i].status === 'pm' || checkData[i].status === 'nok'));
  if (verplicht.length > 0) { naarStap3b(); } else { goStep(3); }
}

function handlePhoto(name, input) {
  if (!input.files[0]) return;
  const reader = new FileReader();
  reader.onload = e => { resizeImage(e.target.result, 1600, 0.85, r => { photoData[name] = r; buildPhotos(); }); };
  reader.readAsDataURL(input.files[0]);
}

// Verkleint foto's zodat de mail klein en snel blijft. Bij fout: originele foto.
...
function goStep(n) {
  ...
  document.getElementById('step-count').textContent = n === 2 ? `${ok}✓ ${pm}!` : '';
...
function buildOverzicht() {
  ...
  // Aandachtspunt + niet OK foto's
  const verplichtItems = checkItems.filter(i => checkData[i] && (checkData[i].status === 'pm' || checkData[i].status === 'nok'));
  let apFotoItems = verplichtItems.map(n =>
    photoData['ap-' + n]
      ? `<div style="text-align:center"><img class="foto-thumb" src="${photoData['ap-'+n]}" alt="${n}" title="${n}"><div style="font-size:9px;color:var(--gray-400);margin-top:3px;max-width:60px;line-height:1.2">${n}</div></div>`
      : `<div class="foto-empty" style="border:1px dashed ${checkData[n].status==='nok'?'#E24B4A':'#BA7517'}">${n}</div>`
  ).join('');

  const totalFotos = photoNames.filter(p=>photoData[p]).length + verplichtItems.filter(i=>photoData['ap-'+i]).length;
  const totalVerwacht = photoNames.length + verplichtItems.length;
  ...
  document.getElementById('overzicht-content').innerHTML = `
    ...
    <div class="ov-card">
      <div class="ov-head">📷 Foto's (${totalFotos} van ${totalVerwacht})</div>
      <div class="foto-row">${fotoItems}</div>
      ${apFotoItems ? `<div style="padding:4px 14px 6px;font-size:11px;font-weight:600;color:var(--gray-400);text-transform:uppercase;letter-spacing:0.05em">Aandachtspunten / Niet OK</div><div class="foto-row">${apFotoItems}</div>` : ''}
    </div>
    <div class="ov-card">
...
async function submitForm() {
  ...
  // 1. Genereer PDF (wordt als bijlage meegestuurd)
  let samenvatting = `Opleverrapport badkamer voor ${document.getElementById('klant').value||'onbekend'}. Status: ${statusTxt}. Checklist: ${ok.length} OK, ${pm.length} aandachtspunten, ${nok.length} niet OK.`;
  let pdfBase64 = '';
  let pdfNaam = '';
  try {
    ...
    let y = 0;
    const GREEN = [29,158,117], GREEN_LIGHT = [225,245,238];
    const RED = [226,75,74], RED_LIGHT = [252,235,235];
    const AMBER = [186,117,23], AMBER_LIGHT = [250,238,218];
    const GRAY_900 = [28,28,26], GRAY_600 = [95,94,90], GRAY_100 = [240,239,234], WHITE = [255,255,255];

    function checkY(n) { if (y + n > 275) { doc.addPage(); y = 16; } }
    function sanitize(str) { return (str||'').replace(/[^\x00-\x7F]/g, '').trim() || '—'; }
    function sectionHeader(title) {
      checkY(12);
      doc.setFillColor(...GRAY_100); doc.rect(margin, y, contentW, 8, 'F');
      doc.setFont('helvetica','bold'); doc.setFontSize(9); doc.setTextColor(...GRAY_600);
      doc.text(title.toUpperCase(), margin+3, y+5.5); y += 10;
    }
    function infoRow(label, value) {
      ...
      doc.setDrawColor(...GRAY_100); doc.line(margin, y+6.5, margin+contentW, y+6.5); y += 7;
    }

    // Header
    doc.setFillColor(...GREEN); doc.rect(0,0,W,24,'F');
    doc.setFont('helvetica','bold'); doc.setFontSize(14); doc.setTextColor(...WHITE);
    doc.text('WBP Poortvliet', margin, 10);
    doc.setFontSize(10); doc.setFont('helvetica','normal');
    doc.text('Opleverrapport Badkamer', margin, 17);
    let statusColor = nok.length>0?RED:pm.length>0?AMBER:GREEN;
    let statusBg = nok.length>0?RED_LIGHT:pm.length>0?AMBER_LIGHT:GREEN_LIGHT;
    doc.setFillColor(...statusBg); doc.roundedRect(W-margin-60,7,60,10,3,3,'F');
    doc.setFont('helvetica','bold'); doc.setFontSize(9); doc.setTextColor(...statusColor);
    doc.text(statusTxt, W-margin-30, 13.5, {align:'center'});
    y = 32;

    // Projectgegevens
    sectionHeader('Projectgegevens');
    infoRow('Klant', sanitize(document.getElementById('klant').value));
    infoRow('Ordernummer', sanitize(document.getElementById('order').value));
    const datum = document.getElementById('datum').value;
    infoRow('Datum', datum ? new Date(datum).toLocaleDateString('nl-NL',{day:'numeric',month:'long',year:'numeric'}) : '—');
    infoRow('Installateur', sanitize(installateurNaam));
    infoRow('Aannemer', sanitize(document.getElementById('aannemer').value));
    infoRow('Opdrachtgever aanwezig', sanitize(document.getElementById('opdrachtgever').value));
    y += 4;

    // Checklist
    sectionHeader(`Checklist — ${ok.length} OK · ${pm.length} Aandachtspunten · ${nok.length} Niet OK`);
    checkItems.forEach(item => {
      checkY(7);
      const s = checkData[item].status, note = checkData[item].note;
      let bgC=GRAY_100, txC=GRAY_600, sym='—';
      if(s==='ok'){bgC=GREEN_LIGHT;txC=GREEN;sym='OK';}
      if(s==='nok'){bgC=RED_LIGHT;txC=RED;sym='NOK';}
      if(s==='pm'){bgC=AMBER_LIGHT;txC=AMBER;sym='!';}
      doc.setFillColor(...bgC); doc.roundedRect(margin,y,10,6,1,1,'F');
      doc.setFont('helvetica','bold'); doc.setFontSize(7); doc.setTextColor(...txC);
      doc.text(sym, margin+5, y+4.3, {align:'center'});
      doc.setFont('helvetica','normal'); doc.setFontSize(8.5); doc.setTextColor(...GRAY_900);
      doc.text(sanitize(item), margin+13, y+4.3);
      if(note){ doc.setTextColor(...GRAY_600); doc.setFontSize(7.5); doc.text(sanitize(note), W-margin-2, y+4.3, {align:'right'}); }
      doc.setDrawColor(...GRAY_100); doc.line(margin,y+6.5,margin+contentW,y+6.5); y+=7;
    });
    y += 4;
    ...
    // Foto's — 1 per pagina
    const fotosMade2 = photoNames.filter(p=>photoData[p]);
    const apFotosMade = checkItems.filter(i => checkData[i] && (checkData[i].status === 'pm' || checkData[i].status === 'nok') && photoData['ap-'+i]);
    const alleFotos = [
      ...fotosMade2.map(n=>({name:n, data:photoData[n], verplicht:false})),
      ...apFotosMade.map(n=>({name:n, data:photoData['ap-'+n], verplicht:true, status:checkData[n].status}))
    ];

    if(alleFotos.length>0){
      alleFotos.forEach(({name, data, verplicht, status}, idx) => {
        doc.addPage(); y = 16;

        // Titel
        doc.setFont('helvetica','bold'); doc.setFontSize(11); doc.setTextColor(...GRAY_900);
        doc.text(sanitize(name), margin, y+6);
        if(verplicht){
          const lc = status==='nok'?RED:AMBER;
          doc.setFontSize(9); doc.setTextColor(...lc);
          doc.text(status==='nok'?'✗ NIET OK':'! AANDACHTSPUNT', W-margin, y+6, {align:'right'});
        }
        y += 12;

        // Foto groot
        const fh = 190;
        try{ doc.addImage(data,'JPEG',margin,y,contentW,fh,undefined,'NONE'); }catch(e){}

        if(verplicht){
          const lb = status==='nok'?RED_LIGHT:AMBER_LIGHT;
          const lc = status==='nok'?RED:AMBER;
          doc.setFillColor(...lb); doc.rect(margin,y+fh,contentW,8,'F');
          doc.setFont('helvetica','bold'); doc.setFontSize(9); doc.setTextColor(...lc);
          doc.text(status==='nok'?'✗ Niet OK — actie vereist':'! Aandachtspunt — controleer dit', margin+contentW/2, y+fh+5.5, {align:'center'});
        }
      });
      // Nieuwe pagina na foto's voor de rest
      doc.addPage(); y = 16;
    }

    // Opmerkingen
    checkY(20); sectionHeader('Opmerkingen / afspraken');
    doc.setFont('helvetica','normal'); doc.setFontSize(9); doc.setTextColor(...GRAY_900);
    const lines = doc.splitTextToSize(sanitize(document.getElementById('opmerkingen').value)||'Geen opmerkingen', contentW-4);
    lines.forEach(l=>{ checkY(6); doc.text(l,margin+2,y+4); y+=5.5; }); y+=6;

    // AI samenvatting
    checkY(20);
    doc.setFillColor(...GREEN_LIGHT); doc.roundedRect(margin,y,contentW,8,2,2,'F');
    doc.setFont('helvetica','bold'); doc.setFontSize(9); doc.setTextColor(...GREEN);
    doc.text('Samenvatting door Claude AI', margin+3, y+5.5); y+=10;
    doc.setFont('helvetica','normal'); doc.setFontSize(8.5); doc.setTextColor(...GRAY_900);
    const aiLines = doc.splitTextToSize(samenvatting, contentW-4);
    aiLines.forEach(l=>{ checkY(6); doc.text(l,margin+2,y+4); y+=5.5; }); y+=6;

    // Handtekeningen
    checkY(50); sectionHeader('Handtekeningen');
    const sigW=(contentW-8)/2;
    ['sig-opdrachtgever','sig-uitvoerder'].forEach((id,idx)=>{
      const label=idx===0?'Opdrachtgever':'Uitvoerder';
      const naam=idx===0?document.getElementById('naam-opdrachtgever').value:document.getElementById('naam-uitvoerder').value;
      const sx=margin+idx*(sigW+8);
      doc.setFont('helvetica','bold'); doc.setFontSize(8); doc.setTextColor(...GRAY_600);
      doc.text(label,sx,y+4);
      doc.setFont('helvetica','normal'); doc.setTextColor(...GRAY_900);
      doc.text(naam||'—',sx,y+9);
      doc.setDrawColor(...GRAY_100); doc.setFillColor(250,250,250);
      doc.roundedRect(sx,y+11,sigW,28,2,2,'FD');
      const canvas=document.getElementById(id);
      if(canvas){try{doc.addImage(canvas.toDataURL('image/png'),'PNG',sx+2,y+12,sigW-4,26);}catch(e){}}
    });
    y+=46;
    ...
    for(let i=1;i<=pc;i++){
      doc.setPage(i); doc.setFillColor(...GRAY_100); doc.rect(0,287,W,10,'F');
      doc.setFont('helvetica','normal'); doc.setFontSize(7); doc.setTextColor(...GRAY_600);
      doc.text('WBP Poortvliet — Opleverrapport Badkamer',margin,293);
      doc.text(`Pagina ${i} van ${pc}`,W-margin,293,{align:'right'});
    }

    // PDF als base64
    const klant = document.getElementById('klant').value||'onbekend';
    const datumStr = datum?datum.replace(/-/g,''):'datum';
    pdfBase64 = doc.output('datauristring').split(',')[1] || '';
    pdfNaam = `Opleverrapport_${klant}_${datumStr}.pdf`;
  } catch(e) { console.warn('PDF generatie fout:', e); }

  // 1b. Maak ZIP van alle foto's (als base64)
  let zipBase64 = '';
  let zipNaam = '';
  try {
    const zip = new JSZip();
    const klant = document.getElementById('klant').value || 'onbekend';
    const alleFotosZip = [
      ...photoNames.filter(n => photoData[n]).map((n, idx) => ({
        name: `${String(idx+1).padStart(2,'0')}_${n.replace(/[^a-zA-Z0-9]/g,'_')}.jpg`,
        data: photoData[n]
      })),
      ...checkItems.filter(i => checkData[i] && (checkData[i].status === 'pm' || checkData[i].status === 'nok') && photoData['ap-'+i])
        .map((item, idx) => ({
          name: `AP_${String(idx+1).padStart(2,'0')}_${item.replace(/[^a-zA-Z0-9]/g,'_')}_${checkData[item].status==='nok'?'NIET_OK':'PUNT'}.jpg`,
          data: photoData['ap-'+item]
        }))
    ];

    if (alleFotosZip.length > 0) {
      alleFotosZip.forEach(({ name, data }) => {
        const base64 = data.split(',')[1];
        zip.file(name, base64, { base64: true });
      });
      zipBase64 = await zip.generateAsync({ type: 'base64', compression: 'DEFLATE' });
      zipNaam = `Fotos_${klant}_${new Date().toISOString().slice(0,10)}.zip`;
    }
  } catch(e) { console.warn('ZIP fout:', e); }

  // 2. Verstuur PDF + foto's als bijlage via Google Apps Script
  try {
    const mailBody =
`Nieuw opleverrapport ontvangen
...
Opmerkingen:
${document.getElementById('opmerkingen').value || 'Geen'}

De PDF en foto's vind je in de bijlage.`;

    const payload = {
      recipients: ONTVANGERS,
      subject: `Opleverrapport - ${document.getElementById('klant').value||'onbekend'} - ${statusTxt}`,
      body: mailBody,
      pdfBase64: pdfBase64,
      pdfName: pdfNaam,
      zipBase64: zipBase64,
...
function downloadPDF() {
  ...
  const GRAY_900 = [28, 28, 26];
  const GRAY_600 = [95, 94, 90];
  const GRAY_100 = [240, 239, 234];
  ...
  doc.roundedRect(W - margin - 60, 7, 60, 10, 3, 3, 'F');
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(9);
  ...
  sectionHeader('Projectgegevens');
  infoRow('Klant', document.getElementById('klant').value);
  infoRow('Ordernummer', document.getElementById('order').value);
  const datum = document.getElementById('datum').value;
  infoRow('Datum oplevering', datum ? new Date(datum).toLocaleDateString('nl-NL', {day:'numeric',month:'long',year:'numeric'}) : '—');
  infoRow('Installateur', installateurNaam);
  infoRow('Aannemer', document.getElementById('aannemer').value);
  infoRow('Opdrachtgever aanwezig', document.getElementById('opdrachtgever').value);
  y += 4;
  ...
  checkItems.forEach(item => {
    checkY(7);
    const s = checkData[item].status;
    const note = checkData[item].note;

    // Status badge
    let bgColor = GRAY_100, txtColor = GRAY_600, symbol = '—';
    if (s === 'ok')  { bgColor = GREEN_LIGHT; txtColor = GREEN; symbol = '✓'; }
    if (s === 'nok') { bgColor = RED_LIGHT;   txtColor = RED;   symbol = '✗'; }
    if (s === 'pm')  { bgColor = AMBER_LIGHT; txtColor = AMBER; symbol = '!'; }
    ...
    doc.text(symbol, margin + 4, y + 4.3, { align: 'center' });

    doc.setFont('helvetica', 'normal');
    doc.setFontSize(8.5);
    doc.setTextColor(...GRAY_900);
    doc.text(item, margin + 11, y + 4.3);

    if (note) {
      doc.setTextColor(...GRAY_600);
      doc.setFontSize(7.5);
      doc.text(`→ ${note}`, margin + 11, y + 4.3, { align: 'left' });
      // note rechts uitlijnen
      doc.text(note, W - margin - 2, y + 4.3, { align: 'right' });
    }
  ...
  const apFotosMade = checkItems.filter(i => checkData[i] && (checkData[i].status === 'pm' || checkData[i].status === 'nok') && photoData['ap-' + i]);
  const alleFotos = [
    ...fotosMade.map(n => ({ name: n, data: photoData[n], verplicht: false })),
    ...apFotosMade.map(n => ({ name: n, data: photoData['ap-' + n], verplicht: true, status: checkData[n].status }))
  ];

  if (alleFotos.length > 0) {
    checkY(10);
    sectionHeader(`Foto's (${alleFotos.length})`);
    const fotoW = contentW;
    const fotoH = 180; // bijna hele pagina hoogte

    alleFotos.forEach(({ name, data, verplicht, status }, idx) => {
      if (idx > 0) { doc.addPage(); y = 16; }
      ...
      if (verplicht) {
        const labelColor = status === 'nok' ? RED : AMBER;
        doc.setFont('helvetica', 'bold');
        doc.setFontSize(9);
      ...
      // Foto groot
      try {
        doc.addImage(data, 'JPEG', margin, y, fotoW, fotoH, undefined, 'NONE');
      } catch(e) {}

      if (verplicht) {
        const labelBg = status === 'nok' ? RED_LIGHT : AMBER_LIGHT;
        const labelColor = status === 'nok' ? RED : AMBER;
        doc.setFillColor(...labelBg);
        doc.rect(margin, y + fotoH, fotoW, 8, 'F');
        doc.setFont('helvetica', 'bold');
        doc.setFontSize(9);
        doc.setTextColor(...labelColor);
        doc.text(status === 'nok' ? '✗ Niet OK — actie vereist' : '! Aandachtspunt — controleer dit', margin + fotoW / 2, y + fotoH + 5.5, { align: 'center' });
      }

      y = 297; // reset voor volgende pagina
    });
    y += 4;
  }
  ...
  checkY(20);
  sectionHeader('Opmerkingen / afspraken');
  doc.setFont('helvetica', 'normal');
  doc.setFontSize(9);
  doc.setTextColor(...GRAY_900);
  const lines = doc.splitTextToSize(opmerkingen || 'Geen opmerkingen', contentW - 4);
  lines.forEach(line => {
    checkY(6);
  ...
  const samenvattingTxt = aiSummaryEl ? aiSummaryEl.textContent : '';
  if (samenvattingTxt && samenvattingTxt !== 'Laden...') {
    checkY(20);
    ...
    doc.text('Samenvatting door Claude AI', margin + 3, y + 5.5);
    y += 10;
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(8.5);
    doc.setTextColor(...GRAY_900);
    const aiLines = doc.splitTextToSize(samenvattingTxt, contentW - 4);
  ...
  ['sig-opdrachtgever', 'sig-uitvoerder'].forEach((id, idx) => {
    ...
    if (canvas) {
      try {
        ...
        doc.addImage(imgData, 'PNG', sx + 2, y + 12, sigW - 4, 26);
      } catch(e) {}
    }
  });
  y += 46;
  ...
  const klant = document.getElementById('klant').value || 'onbekend';
  const datumStr = datum ? datum.replace(/-/g, '') : 'datum';
  doc.save(`Opleverrapport_${klant}_${datumStr}.pdf`);
}

function downloadFotos() {
  const klant = document.getElementById('klant').value || 'onbekend';
  const alleFotos = [
    ...photoNames.filter(n => photoData[n]).map(n => ({ name: n, data: photoData[n] })),
    ...checkItems.filter(i => checkData[i] && (checkData[i].status === 'pm' || checkData[i].status === 'nok') && photoData['ap-' + i])
      .map(i => ({ name: i + ' (' + (checkData[i].status === 'nok' ? 'Niet OK' : 'Punt') + ')', data: photoData['ap-' + i] }))
  ];

  if (alleFotos.length === 0) { alert('Geen foto\'s beschikbaar.'); return; }

  alleFotos.forEach(({ name, data }, idx) => {
    setTimeout(() => {
      const a = document.createElement('a');
      a.href = data;
      const safeName = name.replace(/[^a-zA-Z0-9 ]/g, '').trim().replace(/ /g, '_');
      a.download = `${klant}_${String(idx+1).padStart(2,'0')}_${safeName}.jpg`;
